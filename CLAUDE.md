...
- Unattended factory material (`factory/skills/{scope-review,build,ship,foreman,hindsight,dream}/` and `factory/references/`) never routes a decision to a person: it takes the recommended option under the policy in `factory/references/factory-run.md` and parks only for that file's fixed list.
  ...
  `factory/skills/hindsight/` and `factory/skills/dream/` are runner-only too: the dream loop (`runner/history.py`, `runner/dream.py`) runs them to label finished runs and to revise the foreman skill.
  Text only interactive `scope` uses goes between `<!-- interactive-only -->` markers; `validate.sh` F03 enforces this.
- The dream loop (`factory dream`) edits only `factory/skills/foreman/SKILL.md` and the `plugins/factory` files the plugin build regenerates from it; it never edits stage skills, references, runner code, or caps, and `factory history` and `factory dream` never write inside a run directory.
...
- `factory history build|ls|label` - Turn finished runs into replay worlds under `~/.factory/history` and label them (labelling is paid); never part of `validate.sh`.
- `factory dream [--rounds N] [--no-deploy]` - Paid dream loop: replays the foreman skill over labelled worlds, revises it, and commits and pushes a revision that wins on held-out worlds; never part of `validate.sh`.
