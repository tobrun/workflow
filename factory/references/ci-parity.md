...
- "Pre-existing" is not a waiver. Prove it by running the same command on the
  merge base in an isolated worktree. If the base also fails, fix it anyway in
  its own commit that cites the merge-base proof: a red required check blocks
  the PR either way. Only a fix that needs credentials or infrastructure the run
  does not have parks the run (factory park list). Do not call the branch
  PR-ready while the required check remains red.
...
Once the PR exists (a factory run authorizes ship to push and open it):
...
3. Repeat until every required check is green or a factory park-list item is
   reached.

Do not stop at "CI restarted": shipping means the checks finished green.
