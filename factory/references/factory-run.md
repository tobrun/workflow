...
- Plan files stay out of commits: the runner excludes `.dev/` from Git, so the spec, reviews, notes, `pr.md`, and result files stay on disk only. Commit code, tests, and `docs/` changes; never force-add anything under `.dev/` (the gate warns), and never edit `.factory/contract.json` after scope.
