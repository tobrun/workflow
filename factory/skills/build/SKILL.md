...
If there is no `spec.md` or its change plan is empty, report `failed` with reason "no spec"; without one there are no layer-tagged scenarios to implement against.
