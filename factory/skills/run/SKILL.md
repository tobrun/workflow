...
7. Record the decision with `run-state.py record {plan}` before acting on it, then act per the failure ladder in [references/judgment.md](references/judgment.md): advance, repair it yourself and re-check, relaunch with guidance, or end the run. Follow the phase's budget and the run's ceiling as printed by `run-state.py show {plan}`; `attempt` never refuses one, so honouring it is yours. On `advance`, when this was not the last declared phase, go straight back to step 1 for the next phase in this same turn - advancing is not a place to stop and check in.
