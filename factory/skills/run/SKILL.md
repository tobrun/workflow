...
4. Run `run-state.py check-result .dev/{plan}/results/{phase}-{attempt}.json` with `{attempt}` = `N`; a missing or unparseable file is a failed attempt with that reason (its own exit code, never a crash). Close the attempt with `run-state.py attempt {plan} {phase} --status {done|failed|stopped} --result .dev/{plan}/results/{phase}-{attempt}.json`, the same path.
