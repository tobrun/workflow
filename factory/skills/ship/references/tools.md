...
- **Secrets**: gitleaks or the ecosystem equivalent over the in-scope files. A found secret is never fix-agent work - stop the gauntlet and park the run immediately (factory park list), because it needs rotation and possibly history rewriting, which the run may not do.
