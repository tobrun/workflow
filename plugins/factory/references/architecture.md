...
Write the `Captured` line with the date and the skill, say in the run summary that the overview is new and where it is, and commit it as its own `docs(architecture): capture the system overview` commit.
