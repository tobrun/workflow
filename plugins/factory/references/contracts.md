...
Contract writes are candidates until approved, like every other durable write in this toolkit.
In a factory run, the stage's contract stands in for approval: `scope` promotes with the operator present, `scope-review` promotes its auto-decided changes, and `build` and `ship` never write contracts.
