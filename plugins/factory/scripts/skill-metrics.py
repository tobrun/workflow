...
`start` snapshots the git state into .dev/.metrics/ and locates the session
transcript under $CLAUDE_CONFIG_DIR (default ~/.claude), anchoring on the transcript line that
...
"n/a" rather than failing the run. Codex sessions have no Claude transcript, so
their tokens show "n/a"; a factory run's run.json is the cross-stage token
authority.
"""
...
def state_dir() -> Path:
    # Under the worktree itself: factory runs share final directory names
    # ("worktree"), so a /tmp path keyed by basename would collide across runs.
    path = repo_root() / ".dev" / ".metrics"
