...
4. Hard constraints:
   ...
   - Run only your own change set's tests while you work, and report their real result. Do not run the spec's Validation block or the repository's broad suite: siblings run concurrently against the same mutable worktree, so a broad run would see half-written sibling files and multiply the load. A red result is a fact to report, not something to hide or paper over.
   - Start no subagents of your own: the runner caps concurrent agents per session, and nested fan-out spends that cap.
...
```json
{
  ...
  "selfValidation": { "commands": ["your change set's own test commands"], "result": "pass | fail", "output": "the tail that matters" },
...
1. Verify rather than trust the reports: once every agent in the wave has finished and its writes have settled, run the spec's Validation block yourself, once - the shared tree now holds the whole wave's changes, so one serialized run covers every change set in it.
