---
name: scope
description: Factory copy of scope - interactively spec a change by interviewing for the real problem, arguing every design decision against alternatives, and writing a gate-clean spec with a change plan at the run's fixed .dev/{plan}/spec.md plus a scope-result.json for the factory runner. Use when the factory runner launches the interactive scope stage of a factory run.
---
...
Before the interview, read the run file per "Factory context" below, then run `python3 {scope-skill-root}/../../scripts/skill-metrics.py start scope` so the retrospective can measure the run.

## Factory context

This is the factory copy of `scope`: the factory runner launched it in a run worktree, and its protocol lives in [../../references/factory-run.md](../../references/factory-run.md).

- Read `.dev/factory-run.json` first. Its `plan` is the fixed `{plan-name}`; never pick another. Its `request_file` holds the full change request - typed, from a file, or rendered from a Jira ticket with its comments - so read it before the first question and treat it as the user's opening ask; when `source.kind` is `jira`, write `Source: {key}` under the spec's date line.
- The interview stays interactive: the operator is present in this session.
- The runner's scope gate rejects any `⚑` and any `[open]` in the final spec. A question the operator cannot answer resolves as a `⊘` line whose because clause names the condition that reopens it.
- Render reports under the run file's `report_dir`.
- Write `.dev/{plan-name}/scope-result.json` as your last action. Never invoke `scope-review` or `build`: the runner performs the handoff once the operator confirms.
...
Take `{plan-name}` from the run file (a kebab-case slug naming the outcome only when no run file exists) and write `spec.md` in the plan directory ([../../references/plan-layout.md](../../references/plan-layout.md)), dated under its title, in three sections: `## Research`, `## Scope`, `## Change plan`.
...
`⚑` marks a line waiting on the user while the interview runs; every one resolves before the run ends, because the factory gate rejects the spec otherwise. The change plan never links a decision that is `[open]` or carries one.
...
Record the repo's real typecheck/test/lint/build commands, its pull-request CI checks (or why it has none), and how to launch it for e2e in `.factory/contract.json` per [../../references/repo-contract.md](../../references/repo-contract.md), discovered from `package.json`, a `Makefile`, CI config, or equivalent - never guess `npm test` into a `pytest` repo; ask if you cannot determine them, and confirm the file with the operator because the runner executes it.
End the scope with a `### Validation` block that renders those commands, naming each contract `id`.
...
4. When the reviewer returns, ask remaining questions and update the doc. Anything still unanswerable becomes a `⊘` line with its reopening condition, never a `⚑`.
...
A bug fix's reproducing scenario adds `[repro]` after its layer tag (`[unit] [repro] duplicate delivery -> processed once`): its test must fail on the base and pass on the change.
At the handoff the runner seals the request, the `⊘` non-goals, and every scenario's layer and text as the approved intent, with ids `S1`, `S2`, ...; unattended stages cannot change them.
...
Full-size changes only. Map the settled spec onto `SPEC_DATA` per [references/data-schema.md](references/data-schema.md) and render [templates/spec.html](templates/spec.html) to `{report_dir}/{plan-name}-spec.html`, opening and publishing per [../../references/reporting.md](../../references/reporting.md).
...
Write `scope-result.json` per [../../references/factory-run.md](../../references/factory-run.md) as the final action, with `next: "scope-review"` and the decisions, change sets, and scenarios counts.
