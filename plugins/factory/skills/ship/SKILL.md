...
First read the run file per "Factory context", then run `python3 {ship-skill-root}/../../scripts/skill-metrics.py start ship` so the wrap-up can measure this run instead of recalling it.
...
A factory run always runs all three phases. Outside a run, on request, run a single phase: "gauntlet only" (or "harden only") runs phase 1 alone; "review only" runs phase 2 alone - the right mode when mutating fixes are unwanted, such as on someone else's PR. "No PR" or "local only" runs the default flow without phase 3.
...
- No reviewable files: stop with a `blocked` result carrying an `input.unusable` condition. A tiny or huge diff runs without confirmation: the factory already committed to shipping it.
...
Record which lenses you selected and why in the report's Panel line.

### 4. Run the review panel

Run the two batches - all lens agents first, then all verifiers - on the transport selected by [references/orchestration.md](references/orchestration.md), which owns transport choice, the `prompts/` and `results/` batch layout, the one bounded retry of `pending` tasks, and the prompt contracts.
...
```bash
python3 {ship-skill-root}/scripts/aggregate-findings.py aggregate {batch-1} {batch-2} --expected {lenses you selected} \
  --revision "$(git rev-parse HEAD)" --diff {diffFile} --out .dev/{plan-name}/review_N.json
```

It writes the `factory.review/1` record the runner's gate reads: REFUTED findings drop, a CONFIRMED BLOCK stays a blocker, a PLAUSIBLE BLOCK is kept as an explicitly uncertain concern, and duplicates merge by file:line.
Completeness is separate from the verdict: a lens without a valid result, or a BLOCK or CONCERN without a verifier outcome, makes the review incomplete with no verdict, and the gate rejects it.
Rerun only the tasks `aggregate-findings.py pending` lists, once, then aggregate again; `simplify` and `spec-conformance` are mandatory, and the record's `revision` must be the commit the PR ends on, so re-review after any later commit.
...
Write `.dev/{plan-name}/review_N.md` in the shape of [references/report-format.md](references/report-format.md), at the index of its record, taking the verdict and finding sections from `aggregate-findings.py render .dev/{plan-name}/review_N.json`; the gate rejects a Markdown verdict that differs from the record.

### 7. Publish the report

Map the report onto `REVIEW_DATA` per [references/data-schema.md](references/data-schema.md) and render [templates/review.html](templates/review.html) to `{report_dir}/review_N.html`, per [../../references/reporting.md](../../references/reporting.md) (stable review favicon; title names the plan and review number).
...
PASS or CONCERNS ends the loop; a blocker still standing after round 2 is a real blocker; one needing a spec or decision change is auto-decided per the remediation reference, not escalated.
...
1. Commit what the gauntlet left uncommitted on the run branch and push it.
2. Build the Evidence section from the e2e record with `python3 {ship-skill-root}/scripts/pr-evidence.py extract {report_dir}/{plan-name}-e2e.json`, publishing frontend screenshots to the `pr-evidence` branch with its `publish` command; without an e2e record, capture the evidence now per the reference - screenshots for a UI, a labeled before/after pair otherwise, and a red-on-base, green-on-branch reproducing test for every bug fix.
...
For the gauntlet, per tool: violations found, fixed, and surviving (with the decision taken on each); name the tools acquired or built this run and where they live; state the scope honestly - "hardened the diff" is not "hardened the repo".
...
Then write `ship-result.json` per [../../references/factory-run.md](../../references/factory-run.md) as the final action:

- `done` when the PR is open and ready, the final review verdict is PASS or CONCERNS, `pr-evidence.py check` passed, no required check is failing, the tracked worktree is clean, and nothing is unpushed.
- `blocked` otherwise, with the first unmet condition as `reason`: a real blocker keeps the PR draft and carries no condition, so the runner retries; a park-list item (a found secret, missing credentials, an invalidated premise) carries its typed condition, and a missing `gh` is `input.unusable`.
- `pr_url`, `draft`, `next: null`, the metrics counts including `auto_decided`, and park-list items as typed `conditions`; real review blockers stay in the PR's Open calls.

Never invoke another skill; the runner records the PR and ends the run.
