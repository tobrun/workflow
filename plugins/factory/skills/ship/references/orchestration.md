...
Both transports use the same scratch root, `{scratch_dir}/{phase}/` from
`.dev/factory-run.json` (outside a factory run, `/tmp/ship-{session-id}/`),
with a `prompts/` and a `results/` directory per batch: write each agent's full
prompt to `prompts/{safe-agent-name}.md` before launching it. Result files are the
only channel for agent output.

After a batch, run `aggregate-findings.py pending {batch} --expected {names}`: it
lists the tasks with no result, an invalid result, or a result recorded for a prompt
that has since changed. Relaunch exactly those, once; a task still pending after that
leaves the review incomplete, which the runner treats as a failed attempt, never as a
favorable verdict. A batch in flight means the run is not over:
...
4. A missing or unparseable file after one re-read marks only that agent as
   pending; rerun it per `pending` above.
...
If an agent errors or returns something unparseable, `pending` lists it; retry it once.
Never retry a lens more than once, and never stand in for it yourself.
